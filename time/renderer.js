...
function renderState(payload) {
  ...
  els.todaySummary.textContent = payload.todaySummary?.text || 'No desktop sessions captured yet today.';
  ...
  els.dataPaths.innerHTML = `SQLite database:<br><code>${escapeHtml(payload.dbPath || '—')}</code><br><br>Latest export file:<br><code>${escapeHtml(payload.latestExportPath || '—')}</code>`;
}
...
els.openFolderBtn.addEventListener('click', async () => {
  ...
  if (result?.ok) {
    els.dataPaths.innerHTML = `SQLite database:<br><code>${escapeHtml(result.dbPath || '—')}</code><br><br>Latest export file:<br><code>${escapeHtml(result.latestExportPath || '—')}</code><br><br>App data folder:<br><code>${escapeHtml(result.path)}</code>`;
  }
});
